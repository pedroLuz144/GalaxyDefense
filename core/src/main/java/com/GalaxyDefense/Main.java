...
public class Main extends ApplicationAdapter {
    ...
    private int score, power, numInimigos, level; 

    private FreeTypeFontGenerator generator;
    private FreeTypeFontGenerator.FreeTypeFontParameter parameter, parameterLevel, parameterHighScore, parameterScoreAtual;
    ...
    public void render() {
        ...
        if(!gameOver) {
            ...
            // High Score
            bitmapHighScore.draw(batch, "3450", 1220, 500);
    ...
    private void moveInimigos() {
        ...
        for(Iterator<Rectangle> iter = inimigos.iterator(); iter.hasNext();) {
            ...
            // Colisão com a bala
            if(colisao(inimigo.x, inimigo.y, inimigo.width, inimigo.height, xBala, yBala, bala.getWidth(), bala.getHeight()) && attack) {
                ++score;
                if(score % 15 == 0) {
                    numInimigos -= 100;
                    ++level;
                ...
                yBala = posY;
            // Colisão com a nave
            } else if(colisao(inimigo.x, inimigo.y, inimigo.width, inimigo.height, posX, posY, nave.getWidth(), nave.getHeight()) && !gameOver) {
                --power;
                if(power <= 0) {
                    gameOver = true;
                }
